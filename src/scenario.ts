 ...
 *
 * 事件序列（逻辑时间固定，可稳定重放）：
 *   +0.0s  #101 新字幕（主播林晚，zh-CN，0~3s）
 *   +1.5s  #103 新字幕（#102 尚未到达，时间线出现缺口）
 *   +3.0s  #103 重复推送（同一事件 ID，应被去重）
 *   +4.5s  #102 晚到（应自动补回缺口）
 *  +10.0s  #102 的机器修订 v2（若已锁定则转入人工裁决；时间窗 9~12s）
 *
 * 修订故意留足 5.5 秒间隔，方便运营在修订到达前暂停、编辑并锁定 #102。
 * 说话人 / 语言 / 时间窗元数据供版本化术语表做范围判定。
 */
...
export const SCENARIO_HINT =
  ...
  '右侧「术语校对」面板可维护节目术语表：先看影响范围（自动建议 / 锁定冲突 / 同音歧义 / 不适用），再决定单条或批量接受，支持撤销重做，刷新后自动恢复。'
